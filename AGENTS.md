...
- `cmake/`：CMake 辅助脚本。
- `build/`：本地构建产物和拉取的依赖，视为生成目录，不提交构建工件。
...
- `cmake --build --preset release`：编译优化版二进制。
- `cmake --build build --config Debug --target TopOptFrameC`：仅构建主程序目标。

Windows 下建议通过 IDE 运行；或在 Debug 构建后直接执行 `build/Debug/TopOptFrameC.exe`。

## 代码风格与命名规范
- 语言标准：C++17（`CMAKE_CXX_STANDARD 17`）。
- 缩进：4 个空格，不使用 Tab。
- 类型/类名：`PascalCase`（如 `Application`、`NodeEditor`）。
- 方法/变量：方法使用 `camelCase`；成员变量以下划线结尾（如 `window_`、`cmdHistory_`）。
...
- 提交 PR 前至少验证：Debug 可编译，关键 UI 流程可用（打开/保存工程、节点执行、模型加载）。

## 提交与合并请求规范
当前工作区快照不含 Git 历史，建议采用以下约定：
